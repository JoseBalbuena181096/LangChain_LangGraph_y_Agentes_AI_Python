...
def procesar_entrada():
    ...
    if archivo_cv is not None:
        ...
        if archivo_cv.size > 200 * 1024 * 1024:  # 200 MB
            st.error("❌ El archivo es demasiado grande. El tamaño máximo permitido es 200 MB.")
            return
        
        if archivo_cv.size == 0:
            st.error("❌ El archivo está vacío. Por favor selecciona un archivo PDF válido.")
            return
        ...
        if archivo_cv.type != "application/pdf":
            st.warning("⚠️ El archivo seleccionado podría no ser un PDF válido.")
