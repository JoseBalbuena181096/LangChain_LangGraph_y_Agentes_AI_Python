...
def display_results(result: State, meeting_num: int):
    ...
    output_content = f"""RESULTADOS - REUNIÓN #{meeting_num}
{'='*60}
...
"""
    ...
    if result['action_items']:
        ...
        output_content += "• No se definieron acciones específicas\n"
    
    output_content += f"\nMINUTA FORMAL:\n{'-'*40}\n{result['minutes']}\n{'-'*40}\n"
    output_content += f"\nRESUMEN EJECUTIVO:\n{result['summary']}\n"
    
    output_file = f"reunion_{meeting_num}_resultados.txt"
